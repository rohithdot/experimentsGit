...
remove(P,(1,1),[X1|R],[P|R]) :- X1 = _Z1.
remove(P,(2,1),[X1,X2|R],[X1,P|R]) :- X2 = _Z2.
remove(P,(3,1),[X1,X2,X3|R],[X1,X2,P|R]) :- X3 = _Z3.
remove(P,(1,2),[X1,X2,X3,X4|R],[X1,X2,X3,P|R]) :- X4 = _Z4.
remove(P,(2,2),[X1,X2,X3,X4,X5|R],[X1,X2,X3,X4,P|R]) :- X5 = _Z5.
remove(P,(3,2),[X1,X2,X3,X4,X5,X6|R],[X1,X2,X3,X4,X5,P|R]) :- X6 = _Z6.
remove(P,(1,3),[X1,X2,X3,X4,X5,X6,X7|R],[X1,X2,X3,X4,X5,X6,P|R]) :- X7 = _Z7.
remove(P,(2,3),[X1,X2,X3,X4,X5,X6,X7,X8|R],[X1,X2,X3,X4,X5,X6,X7,P|R]) :- X8 = _Z8.
remove(P,(3,3),[X1,X2,X3,X4,X5,X6,X7,X8,X9|R],[X1,X2,X3,X4,X5,X6,X7,X8,P|R]) :- X9 = _Z9.
...
ttt3(X,Y,P,Q) :-
   ...
   write(OStream,(X,Y,U,V,P,Q)),
   nl(OStream), flush_output(OStream),
   ttt1.

ttt2(P,Q) :-
	board(B),
    alpha_beta(x,1,B,-200,200,(X,Y),_Value),
    record(x,X,Y), 
    retract(board(B)),
	remove(z,(P,Q),A,B),
	assert(board(A)),
